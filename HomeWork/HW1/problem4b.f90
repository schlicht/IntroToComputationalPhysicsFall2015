...
!####################################
      ...
      Real*8, Dimension(1:20) :: xvalues, errorvalues
      Integer :: l, N, m
...
!between our sine and the computer's sine
      ...
      Do l = 0, 20
      xvalue = pi*l/10-pi
      error = sinN(xvalue, m) - sin(xvalue)
      xvalues(m) = m
      errorvalues(m) = 0
!Checks if the error is greater than the previous error for this N
      If (abs(log(abs(error))) > abs(errorvalues(m))) then
!Stores the log of the error error if larger
        errorvalues(m) = log(abs(error))
      End if
      End Do
!Writes the error values
      Write(*,*) xvalues(m), errorvalues(m)  
      End Do
!The machine accuracy is 
      Write(*,*) 'The machine accuracy for our sine function is', &
      'therefore N=16'
      Contains
